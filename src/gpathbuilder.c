...
GPathBuilder* gpath_builder_create(uint32_t max_points)
{
    ...
    APP_LOG(APP_LOG_LEVEL_INFO, "builder: gpath_builder_create");
    return result;
}

void gpath_builder_destroy(GPathBuilder *builder)
{
    APP_LOG(APP_LOG_LEVEL_INFO, "builder: gpath_builder_destroy");
    free(builder);
}
...
GPath* gpath_builder_create_path(GPathBuilder *builder) {
  ...
  {
      APP_LOG(APP_LOG_LEVEL_INFO, "builder: gpath_builder_create_path. num_points <= 1");
      return NULL;
  }
  ...
  if (!result)
  {
      APP_LOG(APP_LOG_LEVEL_INFO, "builder: gpath_builder_create_path. RESULT IS NULL");
      return NULL;
  }
  ...
  memcpy(result->points, builder->points, size_of_points);
    APP_LOG(APP_LOG_LEVEL_INFO, "builder: gpath_builder_create_path");
  return result;
}
...
bool gpath_builder_move_to_point(GPathBuilder *builder, GPoint to_point)
{
  ...
  {
      APP_LOG(APP_LOG_LEVEL_INFO, "builder: num_points != 0");
      return false;
  }
...
bool gpath_builder_line_to_point(GPathBuilder *builder, GPoint to_point)
{
  ...
  {
      APP_LOG(APP_LOG_LEVEL_INFO, "builder: num_points >= max_points-1");
      return false;
  }
