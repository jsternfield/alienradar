...
void draw_ten_percent(GContext *ctx, int x_start, int y_start, int width, int height, GColor first, GColor second)
{
	for(int x = x_start; x < x_start+width; x++)
    {
        ...
        {
			if((x%8 == 0 && y%4 == 0) || ((x+4)%8 == 0 && (y+2)%4 == 0))
            {
...
void draw_forty_percent(GContext *ctx, int x_start, int y_start, int width, int height, GColor first, GColor second)
{
	for(int x = x_start; x < x_start+width; x++)
    {
		for(int y = y_start; y < y_start+height; y++)
        {
			if( (x+y)%2 == 0 && ( (x+y)%8 != 0 || ( (x+y)%8 == 0 && x%4 != 1 && y%4 != 3 ) ) )
            {
                graphics_context_set_stroke_color(ctx, second);
            }
			else
...
void draw_dithered_rect_int(GContext *ctx, GRect bounds, GColor first_color, GColor second_color, int percentage)
{
    DitherPercentage ditherPct = getDitherFromPercentage(percentage);
    draw_dithered_rect(ctx, bounds, first_color, second_color, ditherPct);
}
...
void draw_random_gradient_rect(GContext *ctx, GRect bounds, GColor first_color, GColor second_color, GradientDirection gradient)
{
	...
	else if (gradient == BOTTOM_TO_TOP)
    {
        draw_random_gradient_rect(ctx, bounds, second_color, first_color, TOP_TO_BOTTOM);
    }
	else if (gradient == RIGHT_TO_LEFT)
    {
        draw_random_gradient_rect(ctx, bounds, second_color, first_color, LEFT_TO_RIGHT);
    }
}
